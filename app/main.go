...
func parseIndexInteriorCellData(cellContent []byte, file *os.File) IndexInteriorCell {
	...
	_ = payloadHeaderSize
	cellKeySerialType, n2 := decodeVarint(payload[bytesUsed:])
	fmt.Println("cellKeySerialType", cellKeySerialType)
	keyBytes, _ := getColumnValue(payload[bytesUsed+n2:], uint64(cellKeySerialType))
	if keyBytes != nil {
		indexInteriorCell.key = keyBytes.([]byte)
		fmt.Println(len(indexInteriorCell.key))
	} else {
...
func getIndexLeafPageId(queryToken queryToken, pageId uint32, file *os.File) (uint32, bool, error) {
	...
	if indexPageContent[0] == B_TREE_PAGE_TYPES.INDEX_LEAF_PAGE {
		...
		for i := 0; i < len(cellOffSetsArray); i++ {
			...
			fmt.Println("filter value ", queryToken.filterValue, " key ", key)
