...
func parseCellData(cellContent []byte, tableColumnArray []string) map[string]interface{} {
	// fmt.Println(string(cellContent))
	...
	//get the serial types
	var serialTypes []uint64
	i := 0
	for k < int(headerSize) {
		serialType, l := decodeVarint(header[k:])
		serialTypes = append(serialTypes, uint64(serialType))
		k += l
		i++
	}

	columnNameToSerialTypeMap := make(map[string]uint64)

	for i := 0; i < len(tableColumnArray); i++ {
		columnNameToSerialTypeMap[tableColumnArray[i]] = serialTypes[i]
	}

	payloadBody := payload[headerSize:]
	fmt.Println(string(payloadBody))
	payloadIndex := 0
	rowData := make(map[string]interface{})

	for columnsName, serialType := range columnNameToSerialTypeMap {
		fmt.Println(columnsName, serialType)
		val, k := getColumnValue(payloadBody[payloadIndex:], serialType)
		fmt.Println(val)
		payloadIndex += int(k)
		rowData[columnsName] = val
	}

	return rowData
}

func printInterface(key string, val interface{}) {
	// Try to assert as string
	if s, ok := val.(string); ok {
		fmt.Println(key, s)
		return
	}
	// Try to assert as []byte and convert if possible (common from SQLite binary data!)
	if b, ok := val.([]byte); ok {
