...
func parseCellData(cellContent []byte, tableColumnArray []string) map[string]interface{} {
	...
	columnToSerialType := make([]uint64, len(tableColumnArray))

	for i := 0; i < len(tableColumnArray); i++ {
		columnToSerialType[i] = serialTypes[i]
	}
	...
	for i, serialType := range columnToSerialType {
		unparsedBytes := payloadBody[payloadIndex:]
		fmt.Println("unparsedBytes:", unparsedBytes)
